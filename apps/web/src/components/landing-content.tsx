...
export type LandingVariant = "public" | "authed";

export function LandingContent({ variant = "public" }: { variant?: LandingVariant }) {
  const { t } = useI18n();
  const { signInWithGoogle } = useAuth();
  const showSignIn = variant === "public";

  return (
    <div className={`mx-auto w-full max-w-lg px-4 pt-12 ${showSignIn ? "pb-12" : "pb-safe-nav"}`}>
      <script
      ...
      {/* Footer CTA repeat — only for public variant */}
      {showSignIn && (
