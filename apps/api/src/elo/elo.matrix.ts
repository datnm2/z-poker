...
import { computeEloChanges, ELO_SCALE, K, K_LOSS, WINNER_FLAT_BONUS, WINNER_RAW_FLOOR, LOSER_MIN_PENALTY, BUST_MIN_PENALTY } from "./elo.math";

const parseList = (s: string | undefined, fallback: number[]): number[] =>
  s ? s.split(",").map((x) => Number(x.trim())).filter((n) => !Number.isNaN(n)) : fallback;

const SIZES = parseList(process.env.SIZES, [6, 8, 10]);
const BUY_IN = Number(process.env.BUY_IN ?? 100);
const GAPS = parseList(process.env.GAPS, [-300, -200, -100, -50, 0, 50, 100, 200, 300]);
const CHIPS = parseList(process.env.CHIPS, [-100, -80, -50, -20, -1, 20, 50, 80, 150, 300, 500]);
...
function deltaFor(gap: number, chipsDelta: number, N: number): number {
  ...
  const rows = [
    { playerId: "target", chipsEnd: targetChips, elo: FILLER_ELO + gap, currentStreak: 0 },
...
function printMatrix(N: number): void {
  ...
  console.log("Rows = your ELO gap vs table avg. Cols = your chip result (chips_end − buy_in).");
