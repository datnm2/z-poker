...
export default function LoginPage() {
  const { signInWithGoogle, isLoggedIn, isLoading } = useAuth();
  const { t } = useI18n();
  ...
  return (
    <div className="relative flex min-h-screen flex-col items-center justify-center bg-slate-900 px-4">
      ...
      <div className="w-full max-w-sm space-y-8 text-center">
        <div>
          <h1 className="text-4xl font-bold text-white">Z-Poker</h1>
          <p className="mt-2 text-slate-400">{t("login.tagline")}</p>
        </div>
        ...
        <p className="text-xs text-slate-500">{t("login.domainHint")}</p>
      </div>
      <BottomNav />
    </div>
  );
